...
def init(client_instance):
    ...
    description = "📡 Shona Bot Broadcast System - Advanced message broadcasting with intelligent targeting"
    add_handler("broadcast", commands, description)  # Short, professional button name

async def register_commands():
    """
    Shona Bot broadcast system with advanced targeting
    """
    ...
    async def cipher_elite_broadcast(event):
        try:
            if not event.reply_to_msg_id:
                await event.reply("🎭 **Shona Bot Broadcast System**\n\n"
            ...
            if not args:
                await event.reply("❌ **shona Elite Error:** Please specify target (all/groups/users)")
                return
            
            target_type = args[0].lower()
            
            if target_type not in ["all", "groups", "users"]:
                await event.reply("🎭 **shona bot Broadcast Error**\n\n"
                                "❌ **Invalid target type!**\n\n"
                                "**Valid targets:**\n"
                                "• `all` - All chats\n"
                                "• `groups` - Groups only\n"
            ...
            if not reply_message:
                await event.reply("❌ **Shona Elite Error:** Could not get replied message!")
                return
            
            # Initial status message
            status_msg = await event.reply("🎭 **Shona Bot Broadcast System**\n\n"
            ...
            if not target_chats:
                await status_msg.edit("🎭 **Shona Bot Broadcast Result**\n\n"
                                     f"❌ **No target chats found for type:** {target_type}\n"
                                     f"💡 **Make sure you have chats of this type**")
                return
            
            await status_msg.edit(f"🎭 **Shona Bot Broadcasting**\n\n"
                                 f"🎯 **Target:** {target_type.title()}\n"
            ...
            result_msg = f"🎭 **Shona Bot Broadcast Complete**\n\n"
            ...
            result_msg += f"🤖 **Powered by @shona_bots**"
            
            await status_msg.edit(result_msg)
            
        except Exception as e:
            await event.reply(f"🎭 **shona bot Broadcast System Error**\n\n"
    ...
    async def broadcast_stats(event):
        try:
            ...
            stats_msg = f"🎭 **Shona Bot Broadcast Statistics**\n\n"
            ...
            stats_msg += f"🤖 **Shona Bot Analytics**"
